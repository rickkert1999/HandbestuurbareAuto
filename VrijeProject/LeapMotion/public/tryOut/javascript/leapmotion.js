...
Leap.loop(options, function (frame) {
  ...
  for (var i = 0, len = frame.hands.length; i < len; i++) {
    ...
    handString = console.log("hand_type", hand.type);

    //if this is close to 1 its moste accurate
    handString += concatData("confidence", hand.confidence);
    handString += console.log("confidence", hand.confidence);

    //Pinch strength of hand
    handString += concatData("pinch_strength", hand.pinchStrength);
    handString += console.log("pinch_strength", hand.pinchStrength);

    //Grab strength of hand
    handString += concatData("grab_strength", hand.grabStrength);
    handString += console.log("grab_strength", hand.grabStrength);

    //Positon of hand palm
